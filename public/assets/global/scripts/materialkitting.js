$( function() {
	...
	$('#updateKityQty').on('click', function(event) {
		var kqty = cleanQty($('#getkitQty').val());
		var kitqty = parseFloat(kqty);

		var pqty = cleanQty($('#poqty').val());
		var poqty = parseFloat(pqty);
		...
		if ($('input[name=kitno]').val() == '' || $('input[name=preparedby]').val() == '') {
			...
			if ($('#save_type').val() == 'KIT') {
				var data = {
					...
					poqty: cleanQty($('input[name=poqty]').val()),
					kitqty: cleanQty($('input[name=kitqty]').val()),
	...
	$('#btn_add_issuance').on('click', function() {
		var rowcount = $('#tbl_issuance_body >tr').length;
		rowcount++;
...
function getFifoTable(data) {
	...
	$.each(data, function(i, x) {
		...
		tbl_fifo_body = '<tr style="background-color:'+color+'; color:'+font+'">'+
							'<td width="7.28%">'+
