$(function() {
	getMaterialReturnData();
	checkAllCheckboxesInTable('.group-checkable','.checkboxes');

	$('#btn_add').on('click', function() {
		addState();
	});

	$('#btn_edit').on('click', function() {
		editState();
	});

	$('#btn_save').on('click', function() {
		openloading();
		var data = {
			_token: token,
			id: $('#ret_info_id').val(),
			controlno: $('#controlno').val(),
			po: $('#po').val(),
			date_returned: $('#date_returned').val(),
			remarks: $('#remarks').val(),
			returned_by: $('#returned_by').val(),
			detail_id: $('input[name="details_id[]"]').map(function(){return $(this).val();}).get(),
			issuanceno: $('input[name="details_issuanceno[]"]').map(function(){return $(this).val();}).get(),
			item: $('input[name="details_item[]"]').map(function(){return $(this).val();}).get(),
			item_desc: $('input[name="details_item_desc[]"]').map(function(){return $(this).val();}).get(),
			lot_no: $('input[name="details_lot_no[]"]').map(function(){return $(this).val();}).get(),
			issued_qty: $('input[name="details_issued_qty[]"]').map(function(){return $(this).val();}).get(),
			required_qty: $('input[name="details_required_qty[]"]').map(function(){return $(this).val();}).get(),
			return_qty: $('input[name="details_return_qty[]"]').map(function(){return $(this).val();}).get(),
			actual_returned_qty: $('input[name="details_actual_qty[]"]').map(function(){return $(this).val();}).get(),
			details_remarks: $('input[name="details_remarks[]"]').map(function(){return $(this).val();}).get(),
		}

		$.ajax({
			url: saveMaterialReturnURL,
			type: 'POST',
			dataType: 'JSON',
			data: data,
		}).done(function(data,textStatus,jqXHR) {
			closeloading();
			getMaterialReturnData('',$('#controlno').val());
			msg(data.msg,data.status);
		}).fail(function(data,textStatus,jqXHR) {
			closeloading();
			msg("There was an error occurred.","failed");
		});
	});

	$('#btn_add_details').on('click', function() {
		$('#DetailsModal').modal('show');
	});

	$('#btn_save_details').on('click', function() {
		var total_returned_qty = parseInt($('#total_returned_qty').val()) + parseInt($('#actual_returned_qty').val());
		if (total_returned_qty > $('#return_qty').val()) {
			msg("Actual quantity already exceeded from return quantity.",'failed');
		} else {
			var tbl_details_body = '';
			tbl_details_body = '<tr>'+
									'<td width="4.09%"></td>'+
	            					'<td width="9.09%">'+$('#issuance_no').val()+
	            						'<input type="hidden" name="details_issuanceno[]" value="'+$('#issuance_no').val()+'"/>'+
	            					'</td>'+
	            					'<td width="9.09%">'+$('#item').val()+
	            						'<input type="hidden" name="details_item[]" value="'+$('#item').val()+'"/>'+
	            					'</td>'+
	            					'<td width="16.09%">'+$('#item_desc').val()+
	            						'<input type="hidden" name="details_item_desc[]" value="'+$('#item_desc').val()+'"/>'+
	            					'</td>'+
	            					'<td width="9.09%">'+$('#lot_no').val()+
	            						'<input type="hidden" name="details_lot_no[]" value="'+$('#lot_no').val()+'"/>'+
	            					'</td>'+
	            					'<td width="7.09%">'+$('#issued_qty').val()+
	            						'<input type="hidden" name="details_issued_qty[]" value="'+$('#issued_qty').val()+'"/>'+
	            					'</td>'+
	            					'<td width="7.09%">'+$('#required_qty').val()+
	            						'<input type="hidden" name="details_required_qty[]" value="'+$('#required_qty').val()+'"/>'+
	            					'</td>'+
	            					'<td width="7.09%">'+$('#return_qty').val()+
	            						'<input type="hidden" name="details_return_qty[]" value="'+$('#return_qty').val()+'"/>'+
	            					'</td>'+
	            					'<td width="9.09%">'+$('#actual_returned_qty').val()+
	            						'<input type="hidden" name="details_actual_qty[]" value="'+$('#actual_returned_qty').val()+'"/>'+
	            					'</td>'+
	            					'<td width="15.09%">'+$('#detail_remarks').val()+
	            						'<input type="hidden" name="details_remarks[]" value="'+$('#detail_remarks').val()+'"/>'+
	            					'</td>'+
	            					'<td width="7.09%">'+
										'<input type="hidden" name="details_id[]" value="'+$('#detail_id').val()+'"//>'+
									'</td>'+
	            				'</tr>';
	       	$('#tbl_details_body').append(tbl_details_body);
	       	$('#DetailsModal').modal('hide');
		}
	});

	$('#btn_back').on('click', function() {
		viewState();
		getMaterialReturnData('',$('#controlno').val());
	});

	$('#issuance_no').on('change', function() {
		getItemDetails($(this).val());
	});

	$('#btn_search').on('click', function() {
		
	});

	$('#btn_excel').on('click', function() {
		
	});

	$('#tbl_details_body').on('click', '.brcodebtn', function() {
		var id = $(this).attr('data-id');

		if (isOnMobile() == true) {
			printBRcode(id);
		} else {
			printBRcode(id);
			msg("Please use mobile device.",'failed');
		}
	});

	$('#btn_delete_details').on('click', function() {
		$('#delete_modal').modal('show');
	});

	$('#btn_confirm_delete').on('click', function() {
		delete_items('.checkboxes',deleteDetailsURL);
	});

});

function getMaterialReturnData(to,controlno) {
	var data = {
		_token: token,
		controlno: controlno,
		to: to
	}

	$.ajax({
		url: getMaterialReturnDataURL,
		type: 'GET',
		dataType: 'JSON',
		data: data,
	}).done(function(data,textStatus,jqXHR) {
		if (data.length < 1) {
			viewState();
		} else {
			getDataInfo(data.info);
			getDataDetails(data.details);
			viewState();
			getIssuanceno($('#po').val());
		}
	}).fail(function(data,textStatus,jqXHR) {
		msg("There was an error occurred.","failed");
	});
}

function getDataInfo(data) {
	$('#ret_info_id').val(data.id);
	$('#controlno').val(data.controlno);
	$('#po').val(data.po);
	$('#date_returned').val(data.date_returned);
	$('#remarks').val(data.remarks);
	$('#returned_by').val(data.returned_by);
	$('#create_user').val(data.create_user);
	$('#created_at').val(data.created_at);
	$('#update_user').val(data.update_user);
	$('#updated_at').val(data.updated_at);
}

function getDataDetails(data) {
	var tbl_details_body = "";
	var total_returned_qty = 0;
	$('#tbl_details_body').html(tbl_details_body);
	$.each(data, function(i, x) {
		tbl_details_body = '<tr>'+
								'<td width="4.09%">'+
									'<input type="checkbox" class="checkboxes" name="check_id[]" data-qty="'+x.actual_returned_qty+'" value="'+x.id+'"/>'+
								'</td>'+
            					'<td width="9.09%">'+x.issuanceno+'<input type="hidden" name="details_issuanceno[]" value="'+x.issuanceno+'"/></td>'+
            					'<td width="9.09%">'+x.item+'<input type="hidden" name="details_item[]" value="'+x.item+'"/></td>'+
            					'<td width="16.09%">'+x.item_desc+'<input type="hidden" name="details_item_desc[]" value="'+x.item_desc+'"/></td>'+
            					'<td width="9.09%">'+x.lot_no+'<input type="hidden" name="details_lot_no[]" value="'+x.lot_no+'"/></td>'+
            					'<td width="7.09%">'+x.issued_qty+'<input type="hidden" name="details_issued_qty[]" value="'+x.issued_qty+'"/></td>'+
            					'<td width="7.09%">'+x.required_qty+'<input type="hidden" name="details_required_qty[]" value="'+x.required_qty+'"/></td>'+
            					'<td width="7.09%">'+x.return_qty+'<input type="hidden" name="details_return_qty[]" value="'+x.return_qty+'"/></td>'+
            					'<td width="9.09%">'+x.actual_returned_qty+'<input type="hidden" name="details_actual_qty[]" value="'+x.actual_returned_qty+'"/></td>'+
            					'<td width="15.09%">'+x.remarks+'<input type="hidden" name="details_remarks[]" value="'+x.remarks+'"/></td>'+
            					'<td width="7.09%">'+
									'<a href="javascript:;" class="btn btn-link grey-gallery brcodebtn" data-id="'+x.id+'">'+
										'<i class="fa fa-barcode"></i>'+
									'</a>'+
									'<input type="hidden" name="details_id[]" value="'+x.id+'"/>'+
								'</td>'+
            				'</tr>';
       	$('#tbl_details_body').append(tbl_details_body);
      	total_returned_qty += x.actual_returned_qty;
	});
	$('#total_returned_qty').val(total_returned_qty);
}

function navigate(to) {
	getMaterialReturnData(to,$('#controlno').val());
}

function getIssuanceno(po) {
	var data = {
		_token: token,
		po: po
	};
	$.ajax({
		url: getIssuanceNoURL,
		type: 'GET',
		dataType: 'JSON',
		data: data,
	}).done(function(data,textStatus,jqXHR) {
		$('#issuance_no').select2({
				data: data,
				placeholder: "Select an Issuance No."
			});
	}).fail(function(data,textStatus,jqXHR) {
		msg("There was an error occurred.","failed");
	});
}

function getItemDetails(issuanceno) {
	var data = {
		_token: token,
		issuanceno: issuanceno
	};
	$.ajax({
		url: getItemDetailsURL,
		type: 'GET',
		dataType: 'JSON',
		data: data,
	}).done(function(data,textStatus,jqXHR) {
		$('#item').val(data.item);
		$('#item_desc').val(data.item_desc);
		$('#issued_qty').val(data.issued_qty);
		$('#required_qty').val(data.required_qty);
		$('#return_qty').val(data.return_qty);
		$('#lot_no').val(data.lot_no);
	}).fail(function(data,textStatus,jqXHR) {
		msg("There was an error occurred.","failed");
	});
}

function viewState() {
	$('#controlno').prop('readonly', false);
	$('#po').prop('readonly', true);
	$('#date_returned').prop('readonly', true);
	$('#remarks').prop('readonly', true);
	$('#returned_by').prop('readonly', true);

	$('#btn_add').show();
	$('#btn_edit').show();
	$('#btn_save').hide();
	$('#btn_back').hide();
	$('#btn_search').show();
	$('#btn_excel').show();

	$('#btn_add_details').hide();
	$('#btn_delete_details').hide();
}

function addState() {
	$('#controlno').prop('readonly', true);
	$('#po').prop('readonly', false);
	$('#date_returned').prop('readonly', false);
	$('#remarks').prop('readonly', false);
	$('#returned_by').prop('readonly', false);

	$('#btn_add').hide();
	$('#btn_edit').hide();
	$('#btn_save').show();
	$('#btn_back').show();
	$('#btn_search').hide();
	$('#btn_excel').hide();

	$('#btn_add_details').hide();
	$('#btn_delete_details').hide();

    $('#ret_info_id').val('');
    $('#controlno').val("");
    $('#po').val("");
    $('#date_returned').val("");
    $('#remarks').val("");
    $('#returned_by').val("");

    $('#create_user').val("");
    $('#created_at').val("");
    $('#update_user').val("");
    $('#updated_at').val("");

    $('#tbl_details_body').html("");
}

function editState() {
	$('#controlno').prop('readonly', true);
	$('#po').prop('readonly', true);
	$('#date_returned').prop('readonly', false);
	$('#remarks').prop('readonly', false);
	$('#returned_by').prop('readonly', false);

	$('#btn_add').hide();
	$('#btn_edit').hide();
	$('#btn_save').show();
	$('#btn_back').show();
	$('#btn_search').hide();
	$('#btn_excel').hide();

	$('#btn_add_details').show();
	$('#btn_delete_details').show();
}

function printBRcode(id) {
	window.location.href=barcodeURL+id;
}

function isOnMobile() {
	var isMobile = false; //initiate as false
	// device detection
	if(/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|ipad|iris|kindle|Android|Silk|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(navigator.userAgent) || /1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i.test(navigator.userAgent.substr(0,4))) isMobile = true;

	return isMobile;
}

function delete_items(checkboxClass,url) {
	var chkArray = [];
	$(checkboxClass+":checked").each(function() {
		chkArray.push($(this).val());
	});

	var chkQty = [];
	$(checkboxClass+":checked").each(function() {
		chkQty.push($(this).attr('data-qty'));
	});

	if (chkArray.length > 0) {
		var data = {
			_token: token,
			ids: chkArray,
			qty:chkQty
		}
		delete_now(url,data);
	} else {
		$('#delete_modal').modal('hide');
		msg("Please select at least 1 item.", 'failed');
	}
}

function delete_now(url,data) {
	$.ajax({
		url: url,
		type: 'POST',
		dataType: 'JSON',
		data: data,
	}).done(function(data,textStatus,jqXHR) {
		$('#delete_modal').modal('hide');
		msg(data.msg,data.status);
		getMaterialReturnData('',$('#controlno').val());
	}).fail(function(data,textStatus,jqXHR) {
		msg("There's an error occurred while processing.",'error');
	});
}