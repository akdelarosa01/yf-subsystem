...
<?php $__env->startSection('content'); ?>
	...
	<?php foreach($userProgramAccess as $access): ?>
		<?php if($access->program_code == Config::get('constants.MODULE_CODE_WBSSET')): ?>  <!-- Please update "2001" depending on the corresponding program_code -->
											...
											<?php foreach($tableData as $dest): ?>
												<tr>
													...
													<td style="width: 7%">
							                            ...
							                            <button type="button" name="edit-task" class="btn btn-sm btn-primary edit-task" value="<?php echo e($dest->id . '|' . $dest->name . '|' .$dest->description. '|' .$dest->value); ?>" id="editTask<?php echo e($dest->id); ?>">
						                						<i class="fa fa-edit"></i> 
						            					</button>
						            				</td>
						            				<td><?php echo e($dest->id); ?></td>
													<td><?php echo e($dest->name); ?></td>
													<td><?php echo e($dest->description); ?></td>
													<td><?php echo e($dest->value); ?></td>
												</tr>
											<?php endforeach; ?>
										</tbody>
									</table>									
...
<script type="text/javascript">
	...
	function deleteAllcheckeditems(){
		...
		$.ajax({
			url: "<?php echo e(url('/deleteAll-setting')); ?>",
			method: 'get',
			data:  { 
				tray : tray, 
				traycount : traycount
			...
			/*console.log(data);*/
			window.location.href = "<?php echo e(url('/wbssetiing')); ?>";   
		}).fail(function(jqXHR, textStatus, errorThrown) {
	...
	function Add_Records(){
		...
		if(action == 'ADD')
		{
			...
			$.post("<?php echo e(url('/add-setting')); ?>", 
			{
		...
		{
			$.post("<?php echo e(url('/update-setting')); ?>", 
			{
				_token : $('meta[name=csrf-token]').attr('content')
				, data : myData
			}).done(function(data, textStatus, jqXHR){
