											...
											<tbody id="edit_tbl_body_row">
	                                            ...
	                                            <?php if(isset($packingdetails)): ?>
													...
													<tr id="tr<?php echo $ctr; ?>" >
														...
														<td><?php echo e($details->price); ?><input id="price" type="hidden" class="form-control" name="boxno" value="<?php echo $details->price ?>" <?php echo($readonly); ?>></td>
		...
		function rowClick(ctr)
		{
			...
			if(item.length > 0)
			{
				...
				newrow = '<tr id="tr'+ trcnt +'">'
							...
							+'<td>'+item[2]+'<input id="prodcode" type="hidden" class="form-control" name="boxno" value="'+item[2]+'"></td>'
							+'<td>'+item[3]+'<input id="price" type="hidden" class="form-control" name="boxno" value="'+item[3]+'"></td>'
							+'<td><input id="qty" type="text" class="form-control" name="boxno" value=""></td>'
							+'<td><input id="gross" type="text" class="form-control" name="boxno" value="pcs"></td>'
							// +'<td><a href="#" onclick="edit('+ trcnt +');" value="'+ trcnt +'">Edit</a></td>'
			...
			$('#tbl_editable tr').not(':first').each(function(i, row)
			{
			    ...
			    $(this).find('input').each(function()
			    {
			    	...
			    	if($(this).attr('id') == 'price')
			    	{
			    		if($(this).val().trim() == '')
			    		{
			    			price = '<td style="background-color: red">'+ $(this).val() +'</td>';
			    			err = true;
			    		}
			    		else
			    		{
			    			price = '<td>'+ $(this).val() +'</td>';
			    		}
			    	}
			    	if($(this).attr('id') == 'qty')
			    	{
		...
		function getOrders()
		{
			...
			.done(function(datatable)
			{
				...
				var arr = $.map(datatable, function(datarow)
				{
		    		...
		    		{
		    			if(ckey == 'TPRICE' && value == 0 || ckey == 'BPRICE' && value == 0)
		    			{
