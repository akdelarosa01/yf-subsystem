...
<?php $__env->startSection('content'); ?>
	...
	<?php foreach($userProgramAccess as $access): ?>
		<?php if($access->program_code == Config::get('constants.MODULE_CODE_TRANSET')): ?>  <!-- Please update "2001" depending on the corresponding program_code -->
											...
											<?php foreach($tableData as $dest): ?>
												<tr>
													...
													<td style="width: 5%">
							                           ...
							                           <input type="checkbox" class="form-control input-sm checkboxes" name="checkitem" id="checkitem'.<?php echo e($dest->id); ?>" value="<?php echo e($dest->id); ?>"></input>
						            				</td>
						   															
													<td style="width: 7%">
							                           
							                            <button type="button" name="edit-task" class="btn btn-sm btn-primary edit-task" value="<?php echo e($dest->id . '|' . $dest->code. '|' .$dest->description. '|' .$dest->prefix. '|' .$dest->prefixformat. '|' .$dest->nextno. '|' .$dest->nextnolength); ?>">
						                						<i class="fa fa-edit"></i> 
						            					</button>
						            				</td>
						            				<td><?php echo e($dest->code); ?></td>
													<td><?php echo e($dest->description); ?></td>
													<td><?php echo e($dest->prefix); ?></td>
													<td><?php echo e($dest->prefixformat); ?></td>
													<td><?php echo e($dest->nextno); ?></td>
													<td><?php echo e($dest->nextnolength); ?></td>
												</tr>
											<?php endforeach; ?>
										</tbody>
									</table>									
	...
	<div id="deleteAllModal" class="modal fade" role="dialog">
		<div class="modal-dialog modal-sm gray-gallery">
			<!-- Modal content-->
			<form class="form-horizontal" id="deleteAllform" action="<?php echo e(url('/deleteAll-transaction')); ?>" role="form" method="POST">
				<div class="modal-content">
					<div class="modal-header">
...
<script type="text/javascript">
	$( document ).ready(function(e) {
		...
		$('.delete-task').click(function(e){      
        	...
        	$.ajax({
				url: "<?php echo e(url('/delete-transaction')); ?>",
				method: 'get',
				data:  { masterid : $(this).val() },
				
			}).done( function(data, textStatus, jqXHR) {
				window.location.href = "<?php echo e(url('/transactionsetting')); ?>";   
...
/*	function deleteAll(){
	...
	function deleteAllcheckeditems(){
		...
		$.ajax({
			url: "<?php echo e(url('/deleteAll-transaction')); ?>",
			method: 'get',
			data:  { 
				tray : tray, 
				traycount : traycount
	...
	function Add_Records(){
		...
		if(action == 'ADD')
		{
			$.post("<?php echo e(url('/add-transaction')); ?>", 
			{
		...
		{
			$.post("<?php echo e(url('/update-transaction')); ?>", 
			{
				_token : $('meta[name=csrf-token]').attr('content')
				, data : myData
			}).done(function(data, textStatus, jqXHR){
