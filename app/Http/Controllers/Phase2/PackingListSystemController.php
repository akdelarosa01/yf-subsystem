    ...
    **/
    public function savePackingList(Request $request_data)
    {
        ...
        // {
            ...
            if(empty($details[8]) || $details[8] == 'NA' || $details[8] == 'na' || $details[8] == 'n/a' || $details[8] == 'N/A')
            {
                $r_time      = '';
            }
            else
            ...
            if(empty($details[9]) || $details[9] == 'NA' || $details[9] == 'na' || $details[9] == 'n/a' || $details[9] == 'N/A')
            {
                $r_pickupdate= 'N/A';
            }
            else
            ...
            if(empty($selecteditem))
            {
                ...
                if (count($detailsList) > 0) {
                    ...
                    foreach ($detailsList as $key => $row) {
                        if (strlen($row[1]) == 15 || substr($row[1],0,2) == 'SJ' || substr($row[1],0,2) == 'ES') {
                            $unitprice = $row[4];
                        } else {
                            ...
                            if(strpos($this->getDropdownVal($shipinstruction),'RAW MATERIALS') !== false) {

                                switch (Auth::user()->productline) {
                                    case 'TS':
                                        $percent = $row[4]*0.04;
    ...
    private function insertPackingDetails($detailsList, $selecteditem, $action)
    {
        ...
        try
        {
            if(count($detailsList) >= 1)
            {
                ...
                {
                    if (strlen($detailsRow[1]) == 15 || substr($detailsRow[1],0,2) == 'SJ' || substr($detailsRow[1],0,2) == 'ES') {
    ...
    private function exportPackingList($invoicedate_from, $invoicedate_to)
    {
        try
        {
            if(empty($invoicedate_from) || empty($invoicedate_to))
            {
                $packinglist = DB::connection($this->mysql)->table('tbl_packing_list')
                            ->select(
                            DB::raw("control_no AS 'CTR #'"),
                            DB::raw("DATE_FORMAT(invoice_date, '%m/%d/%Y') as 'Invoice Date'"),
                            DB::raw("CONCAT(remarks_time, '\r\n' ,DATE_FORMAT(remarks_pickupdate, '%m/%d/%Y'), '\r\n',remarks_s_no) AS Remarks"),
                            DB::raw("sold_to AS 'Sold To'"),
                            DB::raw("ship_to AS 'Ship To'"),
                            DB::raw("(SELECT description from pmi_common.tbl_mdropdowns where id = carrier) as Carrier"),
                            DB::raw("DATE_FORMAT(date_ship, '%m/%d/%Y') AS 'Date Ship'"),
                            DB::raw("port_loading AS 'Port of Loading'"),
                            DB::raw("(SELECT description from pmi_common.tbl_mdropdowns where id = port_destination) AS 'Port of Destination'"),
                            DB::raw("CONCAT(`from`, '\r\n' ,`to`, '\r\n',freight) AS 'Shipping Instruction'"),
                            DB::raw("case_marks AS 'Case Marks'"),
                            'note AS Note')
                            ->get();
            }
            else
            {
                ...
                $packinglist = DB::connection($this->mysql)->table('tbl_packing_list')
                            ->select(
                            DB::raw("control_no AS CTRL_NO"),
                            DB::raw("DATE_FORMAT(invoice_date, '%m/%d/%Y') as Invoice_Date"),
                            DB::raw("CONCAT(remarks_time, DATE_FORMAT(remarks_pickupdate, '%m/%d/%Y'), remarks_s_no) AS Remarks"),
                            DB::raw("sold_to AS Sold_To"),
                            DB::raw("ship_to AS Ship_To"),
                            DB::raw("(SELECT description from pmi_common.tbl_mdropdowns where id = carrier) as Carrier"),
                            DB::raw("DATE_FORMAT(date_ship, '%m/%d/%Y') AS Date_Ship"),
                            DB::raw("port_loading AS Port_of_Loading"),
                            DB::raw("(SELECT description from pmi_common.tbl_mdropdowns where id = port_destination) AS Port_of_Destination"),
                            DB::raw("CONCAT(`from`, '\r\n' ,`to`, '\r\n',freight) AS Shipping_Instruction"),
                            DB::raw("case_marks AS Case_Marks"),
                            'note AS Note')
    ...
    **/
    public function exportListToPdfPrint(Request $request_data)
    {
        try
        {
            ...
            $whs = "Whse Supvr / Manager";
            $copy = "Traffic/Prod'n/QC";
