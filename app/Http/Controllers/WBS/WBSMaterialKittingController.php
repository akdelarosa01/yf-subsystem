            ...
            if ($req->iss_db_id == '') {
                ...
                foreach ($req->iss_db_id as $key => $id) {
                    ...
                    DB::connection($this->mysql)->table('tbl_wbs_material_kitting_details')
                        ->where('issue_no',$req->issuanceno)
                        ->where('item',$req->issitem[$key])
                        ->update(['issued_qty' => $iss->issued_qty]);
                }
        ...
        if ($checklot > 0) {
        	$data = DB::connection($this->mysql)->table('tbl_wbs_inventory as i')
		                ...
		                ->select(DB::raw('i.id as id'),
                            ...
                            DB::raw("DATE_FORMAT(i.exp_date, '%m/%d/%Y') as receive_date"),
                            DB::raw('k.kit_qty as kit_qty'))
		                ->orderBy('i.exp_date','asc')
		                ->get();
            // if (count((array)$data) < 1) {
            //     $data = DB::connection($this->mysql)->table('tbl_wbs_inventory as i')
            //                 ->join('tbl_wbs_material_kitting_details as k','i.item','=','k.item')
            //                 ->join('tbl_wbs_local_receiving_batch as r','i.invoice_no','=','r.invoice_no')
            //                 ->whereRaw("i.item=r.item AND i.qty > 0 AND i.for_kitting='1' AND i.for_kitting='1' AND k.issue_no='".$req->issuanceno."'".$lotno_cond.$item_cond) //
            //                 ->select(DB::raw('i.id as id'),
            //                     DB::raw('i.item as item'),
            //                     DB::raw('i.item_desc as item_desc'),
            //                     DB::raw('r.qty as received_qty'),
            ...
            //             ->join('tbl_wbs_material_kitting_details as k','i.item','=','k.item')
            //             ->join('tbl_wbs_local_receiving_batch as r','i.invoice_no','=','r.invoice_no')
            //             ->whereRaw("i.item=r.item AND i.qty > 0 AND i.for_kitting='1' AND i.for_kitting='1' ND k.issue_no='".$req->issuanceno."'".$item_cond)//A
            //             ->select(DB::raw('i.id as id'),
            //                 DB::raw('i.item as item'),
            //                 DB::raw('i.item_desc as item_desc'),
            //                 DB::raw('r.qty as received_qty'),
